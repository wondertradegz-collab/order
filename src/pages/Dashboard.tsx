...
export function Dashboard() {
  const { documents, customers, memos, toggleMemoComplete, activityLogs } = useApp();
      ...
      <Card>
        ...
        {activityLogs.length === 0 ? (
          <EmptyState
            ...
            title={t('dashboard.noActivityLogs')}
            description={t('dashboard.noActivityLogsDescription')}
          />
        ) : (
          <div className="space-y-2 max-h-80 overflow-y-auto">
            {activityLogs.slice(0, 20).map((log) => (
              ...
              >
                <div className={`p-2 rounded-lg flex-shrink-0 ${getActivityIconColor(log.type)}`}>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap">
                    ...
                    {log.details && (
                      <Badge color="gray">{log.details}</Badge>
                    )}
                  </div>
                  <p className="text-sm text-gray-500 dark:text-gray-400">
                    {formatActivityTime(log.timestamp, language)}
                  </p>
                </div>
              </div>
            ))}
...
function getActivityIconColor(type: string): string {
  const colors: Record<string, string> = {
    login: 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400',
    logout: 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400',
    document_create: 'bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-400',
    document_update: 'bg-yellow-100 dark:bg-yellow-900/50 text-yellow-600 dark:text-yellow-400',
    document_delete: 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400',
    customer_create: 'bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400',
    customer_update: 'bg-purple-100 dark:bg-purple-900/50 text-purple-600 dark:text-purple-400',
    customer_delete: 'bg-red-100 dark:bg-red-900/50 text-red-600 dark:text-red-400',
    payment_record: 'bg-green-100 dark:bg-green-900/50 text-green-600 dark:text-green-400',
    settings_update: 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-400',
    backup_create: 'bg-blue-100 dark:bg-blue-900/50 text-blue-600 dark:text-blue-400',
    backup_restore: 'bg-orange-100 dark:bg-orange-900/50 text-orange-600 dark:text-orange-400',
  };
...
function formatActivityTime(timestamp: string, language: string): string {
  ...
  if (diffMins < 1) {
    return language === 'ja' ? 'たった今' : language === 'zh' ? '刚刚' : 'Just now';
  } else if (diffMins < 60) {
    return language === 'ja' ? `${diffMins}分前` : language === 'zh' ? `${diffMins}分钟前` : `${diffMins}m ago`;
  } else if (diffHours < 24) {
    return language === 'ja' ? `${diffHours}時間前` : language === 'zh' ? `${diffHours}小时前` : `${diffHours}h ago`;
  } else if (diffDays < 7) {
    return language === 'ja' ? `${diffDays}日前` : language === 'zh' ? `${diffDays}天前` : `${diffDays}d ago`;
  } else {
    const locale = language === 'zh' ? 'zh-CN' : language === 'en' ? 'en-US' : 'ja-JP';
